...
def build_vector_database(excel_file, output_file):
    ...
    for _, row in df.iterrows():
        chunks = chunk_text(row['title']+row['ile'], 250)
        video_embeddings = []
        i_chunk = 0
        for chunk in chunks:
            i_chunk += 1
        ...
        processed_data.append({
            ...
            'video_knowledgePoints': row['kpk'],
            'video_sub': row['sub']+"_"+row['grade']+"_"+row['vol']+"_"+row['unit'],
            'video_link': row['videoUrl'],
            'video_cover_link': row['coverUrl'],
            'video_keywords': row['kp'],
            # 'video_vid': row['vid'],
            'video_summary': row['ile'],
...
if __name__ == "__main__":
    excel_file = "hjb_sx_1.2.xlsx"
    vector_db_file = "hjb_sx_1.2_vector_database.pkl"
