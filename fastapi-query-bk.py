...
def keyword_search_v2(query, vector_db):
    """Perform keyword search and return matching scores"""
    query_keywords = set(jieba.cut(query.lower()))
    video_scores = []
    
    for video in vector_db:
        video_text = (video['video_name'] + ' ' + video['video_sub'] + ' ' + video['video_keywords'] + ' ' + video['video_knowledgePoints']).lower()
        video_keywords = set(jieba.cut(video_text))
        common_keywords = query_keywords.intersection(video_keywords)
...
async def combined_search_znd_v2(query, vector_db, vector_weight=0.5, keyword_weight=0.5, top_k=3):
    ...
    keyword_scores = keyword_search_v2(query, vector_db)
...
# 加载vector database
vector_db_file_znd = "znd456_vector_database.pkl"
vector_db_file_hjb = "hjb-sx-1.1_vector_database.pkl"
vector_db_file_znd_v2 = "znd456_v2_vector_database.pkl"
vector_db_znd = None
vector_db_hjb = None
vector_db_znd_v2 = None

@asynccontextmanager
async def lifespan(app: FastAPI):
    global vector_db_znd, vector_db_hjb, vector_db_znd_v2
    vector_db_znd = await load_vector_database(vector_db_file_znd)
    vector_db_hjb = await load_vector_database(vector_db_file_hjb)
    vector_db_znd_v2 = await load_vector_database(vector_db_file_znd_v2)
    print("==============================================")
...
async def search(query: Query):
    if not vector_db_znd:
        raise HTTPException(status_code=500, detail="Vector database not loaded")
    
    matches = await find_matching_videos(query.query, vector_db_znd,top_k=query.top_k)
...
async def search(query: Query):
    ...
    matches = await combined_search_znd(query.query, vector_db_znd,vector_weight=0.2, keyword_weight=0.8,top_k=query.top_k)
    return {"results": matches}

@app.post("/mix/aisearchzndv2",tags=["AI-Local-LLM-RAG"],description="基于向量库和关键字的视频推荐混合搜索V2版，实施范围“456年级语数英重难点”，速度较快", summary="视频AI推荐混合搜索-重难点接口V2", response_description="返回视频列表")
async def search(query: Query):
    if not vector_db_znd_v2:
        raise HTTPException(status_code=500, detail="Vector database not loaded")
    
    matches = await combined_search_znd_v2(query.query, vector_db_znd_v2,vector_weight=0.2, keyword_weight=0.8,top_k=query.top_k)
    return {"results": matches}
